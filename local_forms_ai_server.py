...
def local_answer_for_question(question: dict[str, Any], config: dict[str, Any]) -> Any:
    ...
    if "dispuesto a ser lider" in title or "dispuesto a ser líder" in title or "willing to be a leader" in title:
    ...
    if "contacto de emergencia" in title or "persona de contacto" in title or "emergency contact person" in title:
    ...
    if "necesidades personales" in title or "enfermedades cronicas" in title or "enfermedades crónicas" in title or "movilidad reducida" in title:
        return common.get("dietary_requirements_spanish", profile.get("DIETARY_MEDICAL_TRAVEL_REQUIREMENTS", MANUAL_REQUIRED))
